 ...
 * between Flutter and Unreal Engine on macOS.
 */
public class UnrealEngineController: NSObject {
     ...
     */
    public func attachView(to parent: NSView) {
    ...
    private func loadUnrealFramework() -> UnrealFramework? {
        ...
        let bundlePath = Bundle.main.bundlePath + "/Contents/Frameworks/UnrealFramework.framework"
    ...
    private func applyConfiguration(_ config: [String: Any]) {
        ...
        if let enableMetal = config["enableMetal"] as? Bool, enableMetal {
            // Enable Metal graphics API (default on macOS)
            NSLog("[UnrealEngineController] Metal graphics enabled")
        }

        if let enableHighDPI = config["enableHighDPI"] as? Bool, enableHighDPI {
            // Enable high DPI rendering (Retina support)
            NSLog("[UnrealEngineController] High DPI rendering enabled")
        }

        if let enableFullscreen = config["enableFullscreen"] as? Bool, enableFullscreen {
            NSLog("[UnrealEngineController] Fullscreen mode enabled")
        }
    }
     ...
     * Implemented in Objective-C++ bridge
     */
    private func nativeGetView() -> NSView? {
